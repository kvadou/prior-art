...
while getopts "n:s:o:k:" opt; do
  case "$opt" in
    ...
    o) OUTDIR="$OPTARG" ;;
    k) KEYWORDS="$OPTARG" ;;
    *) echo "usage: $0 [-n TOP] [-s MIN_STARS] [-k \"kw1|kw2\"] [-o OUTDIR] \"query\" ..." >&2; exit 2 ;;
  esac
done
shift $((OPTIND - 1))
...
for q in "$@"; do
  ...
  gh api -X GET search/repositories \
    ...
    --jq '.items[] | {full_name, html_url, description, stars: .stargazers_count, forks: .forks_count, language, license: (.license.spdx_id // "NONE"), pushed_at, archived, topics}' \
    >> "$RAW" 2>/dev/null || echo "  (query failed, skipping)" >&2
  sleep 2
done
...
MD="$OUTDIR/$STAMP-report.md"
{
  ...
  echo
  echo "| # | Repo | Stars | Lang | License | Maintained | Rel | Score |"
  echo "|---|------|-------|------|---------|-----------|-----|-------|"
  jq -r --argjson top "$TOP" '.[:$top] | to_entries[] |
    "| \(.key+1) | [\(.value.full_name)](\(.value.html_url)) | \(.value.stars) | \(.value.language // "-") | \(.value.license) | \(if .value.maintained then "yes" else "NO" end) | \(if .value.relevant then "y" else "n" end) | \(.value.score) |"' "$JSON"
  echo
  echo "## Candidates"
  echo
